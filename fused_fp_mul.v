...
module mainMod(A,B,C,op,out,rnd_mode);
//module mainMod(clk);
...
wire [22:0]mantisaTempA,mantisaTempB,mantisaTempC;
wire [23:0]mantisaA,mantisaB,mantisaC;
wire [47:0]multAB,small_csa_in_sum,small_csa_in_cy,large_csa_in_sum,large_csa_in_cy;
wire exp_comp;
...
wire [47:0]normalise_in1,normalise_in2;
...
//calling exp compare
expCompare e1(expA,expB,expC,exp,exp_comp,exp_diff);

//calling op select
opSelect op1(A[31],B[31],C[31],op,op_sel);
wire [47:0] C_ext;
assign C_ext = {mantisaC,24'b0};
//calling 2:1 mux to select Small Number
mux_2_1_47bits a1(multAB,C_ext,exp_comp,small_num,large_num);
...
//calling sign logic to find out sign
 signLogic ss1(A[31],B[31],C[31],exp_comp,signifComp,sign,op);
 
//calling LZA to get norm_shift
leadOne l1(normalise_in1,norm_shift);

//generating path select
pathSelect pp1(expA,expB,expC,path_sel,op_sel);
